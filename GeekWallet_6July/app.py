...
class Budget(db.Model):
    ...
    start_date = db.Column(db.Date, nullable=False)
    budget_note = db.Column(db.String(255))  # luna added 29-May-2026
    budget_noti = db.Column(db.Boolean, default=True)  # luna added 29-May-2026
    end_date = db.Column(
...
def dreports():
    ...
    # 1. DEFINE THE EXPENSE QUERY
    expense_query = text(dedent("""
        SELECT 
            c.ctg_name,
            SUM(t.amount) AS total
        FROM transactions t
        JOIN category c ON t.ctg_id = c.ctg_id
        WHERE t.user_id = :user_id
            AND c.ctg_type = 'Expense'
    ...
    # 2. EXECUTE THE QUERY
    expense_results = db.session.execute(
        expense_query,
    ...
    # 1. INCOME SOURCES DOUGHNUT QUERY
    # =========================================================
    income_src_query = text(dedent("""
        SELECT 
            c.ctg_name,
            SUM(t.amount) AS total
        FROM transactions t
        JOIN category c ON t.ctg_id = c.ctg_id
        WHERE t.user_id = :user_id
            AND c.ctg_type = 'Income'
            AND t.date BETWEEN :start_date AND :end_date
        GROUP BY c.ctg_name;
    """))

    income_src_results = db.session.execute(
        income_src_query,
        {
            "user_id": user_id,
            "start_date": start_date,  # variable name
            "end_date": end_date,  # variable name end_date_str
        },
    ).fetchall()
    ...
    # 3. FORMAT THE DATA FOR YOUR CHART
    ...
    # Note: We swapped out '%s' for secure named parameters like ':user_id'
    weekly_line_query = text(dedent("""
        SELECT
            FLOOR(DATEDIFF(t.date, :start_date1) / 7) + 1 AS week_no,
            c.ctg_type,
            SUM(t.amount) AS total
        FROM transactions t
        JOIN category c ON t.ctg_id = c.ctg_id
        WHERE t.user_id = :user_id
            AND t.date BETWEEN :start_date2 AND :end_date
        GROUP BY week_no, c.ctg_type
        ORDER BY week_no;
    """))

    # Execute the raw query and fetch all matching records
